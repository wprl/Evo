...
public class Solution {
    ...
    public int uncutPaths() {
        if (this.uncutPaths == null) {
            ...
            this.uncutPaths = uncut;
        }

        return this.uncutPaths;
    }

    /**
     * Get the fitness of this solution (not the fitness based on Pareto fronts, the fitness for the solution file)
    ...
    public String toSolutionString(int routerIdOffset) {
        ...
        // Write cardinality
        s.append(this.deactivatedCount());
        s.append("\t");

        // Write number of cut paths
        s.append(this.network.length - this.uncutPaths()); // cut paths = total paths - uncut paths
        s.append("\t");

        // Write deactivated router IDs
        for (int i = 0; i < this.genotype.length; i++) {
            if (!this.genotype[i]) {
     ...
     * @return
     */
    public double distanceFrom(Solution other) {
        double distance = 0.0;
        for (int i = 0; i < this.genotype.length; i++) {
