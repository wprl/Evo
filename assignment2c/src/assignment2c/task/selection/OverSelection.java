...
public class OverSelection extends SelectTree<Solution> {
    ...
    public List<Solution> select(int numberToSelect, List<Solution> pool) {
        ...
        int numberInUpper = (int) ((double) pool.size() * config.getPercentInUpperTierForOverselection()); // x% are in top tier
        ...
        List<Solution> upperTier = sortedPool.subList(numberInLower, numberInLower + numberInUpper);

        int numberFromUpper = (int) ((double) numberToSelect * 0.8); // 80% come from top tier
        int numberFromLower = numberToSelect - numberFromUpper; // 20% from bottom tier
